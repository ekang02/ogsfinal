          ...
          <div class="mt-2">
            ...
            >
              ...
              <template v-slot:cell(actions)="row">
                <button class="btn btn-secondary"  :class="{ 'btn-success': row.item.isPrincipal === 1}" @click="current(row.item.id)">
                  {{ row.item.isPrincipal === 1 ? 'Current' : 'Set as Current' }}
                </button>
              </template>
            </b-table>
...
export default {
  ...
  methods: {
    ...
    current(id){
      var self = this;
      self.$swal({
          title: "Are you sure to set this as Current Principal?",
          ...
          if (result.value) { // <-- if confirmed
            self.$swal({
             ...
             axios.post('update-principal-active', {
                id: id,
              })
              .then(function (response) {
                self.$swal({
