                ...
                <div class="col-sm-12">
                  <div class="row">
                    ...
                    <div class="row justify-content-between col-sm-12">
                      ...
                      <div class="form-group col-sm-6">
                          <label>Section</label>
                          <select class="form-control" v-model="form.section" :class="{ 'is-invalid': attemptSubmit && missingSection }" :disabled="isSectionDisabled">
                            <option v-for="section in sections" :value="section" :key="section.id">{{ section.label }}</option>
                          </select>
                          <div class="invalid-feedback">This field is required.</div>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    getStudentId(){
      var self = this;
      axios.get('/get-student-id')
      .then(function (response) {
        // handle success
        let result = response.data;
        self.form.student_id = result;
      })
      .catch(function (error) {
        console.log(error);
      });
    ...
    validateForm: function (event) {
      ...
      if (this.missingGender || this.missingAddress || this.missingUsername || this.missingFirstName || this.missingLastName || this.missingMobileNumber ||  this.missingEmail || this.missingPassword || this.missingAge || this.missingBirthDate || this.missingYear || this.missingSection){
