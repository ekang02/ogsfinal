...
class PrincipalController extends Controller
{
    ...
    public function getDashboardData(Request $request){
        $current_school_year = AcademicYear::where('isActive', 1)->first();
        $total_student = StudentInfo::where('academic_year_id', $current_school_year['id'])->count();
        $total_student_male = StudentInfo::where('academic_year_id', $current_school_year['id'])->where('gender', 'male')->count();
        $total_student_female = StudentInfo::where('academic_year_id', $current_school_year['id'])->where('gender', 'female')->count();
        $total_teacher = User::where('user_role', 4)->count();
        $arr1 = array('total_student' => $total_student);
        $arr2 = array('total_teacher' => $total_teacher);
        $arr3 = array('total_student_male' => $total_student_male);
        $arr4 = array('total_student_female' => $total_student_female);
        $arr5 = array('current_school_year' => $current_school_year);
        $array = $arr1 + $arr2 + $arr3 + $arr4 + $arr5;
        return $array;
    }
    public function getSectionBarChart(Request $request){
        $current_school_year = AcademicYear::where('isActive', 1)->first();
        if($request['year_id']){
            $sections = Section::where('academic_year_id', $current_school_year['id'])->where('year_id',$request['year_id'])->with('year')->get();
        }else{
            $sections = Section::where('academic_year_id', $current_school_year['id'])->with('year')->get();
        }
        $dataset = array();
        foreach ($sections as $key => $value) {
            $value['labels'] =  $value['year']['year'] . ' - ' . $value['section'] .'('  . $value['label'] .  ')';
            $value['total_student_per_section'] = StudentInfo::where('section_id', $value['id'])->count(); 
